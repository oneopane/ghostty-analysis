...
- ingestion pipeline code (`packages/ingestion/...`),
- runtime SQLite schema generated by `init_db` (verified at `/tmp/attention_contract_full.sqlite`),
- routing/evaluation artifact writers (`packages/inference/...`, `packages/evaluation/...`).
...
- **Entrypoints (Typer CLI):** `packages/ingestion/src/gh_history_ingestion/cli/app.py`
  - `ingestion ingest` → full backfill (`ingest/backfill.py`)
  - `ingestion incremental` → watermark-driven update (`ingest/incremental.py`)
  - `ingestion pull-requests` → PR-window backfill (`ingest/pull_requests.py`, optional `--with-truth`)
...
- **Important:** ingestion package does **not** write `snapshot.json` / `inputs.json`; those are written by routing/eval artifact code (`inference/artifacts/writer.py`).
...
Source of truth: `packages/ingestion/src/gh_history_ingestion/storage/schema.py`, validated against generated DB `/tmp/attention_contract_full.sqlite`.
...
- Verified CLI ingestion entrypoints in `ingestion` (`ingest`, `incremental`, `pull-requests`).
