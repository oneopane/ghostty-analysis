...
import repo_cli.unified_experiment as unified_experiment
...
def test_cohort_create_is_deterministic(tmp_path: Path) -> None:
    ...
    r1 = runner.invoke(app, [*args, "--output", str(out1)])
    r2 = runner.invoke(app, [*args, "--output", str(out2)])
...
def test_experiment_run_uses_spec_locked_cohort_and_cutoffs(
    ...
    monkeypatch.setattr(unified_experiment, "compute_run_id", lambda cfg: "run-fixed")
...
def test_experiment_run_records_prefetch_network_provenance_in_manifest(
    ...
    monkeypatch.setattr(unified_experiment, "_prefetch_missing_artifacts", lambda **kwargs: prefetch_summary)
    monkeypatch.setattr(unified_experiment, "compute_run_id", lambda cfg: "run-prefetch")
    monkeypatch.setattr(
        unified_experiment,
        "run_streaming_eval",
...
def test_experiment_explain_passes_policy_flag(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(unified_experiment, "eval_explain_cmd", _fake_explain)
