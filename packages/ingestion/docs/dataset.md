...
- `commits`: commit metadata (SHAs only).
- `refs`: branch/tag markers pointing to commit SHAs.
- `releases`: release metadata tied to tags.
...
### Operational Tables
- `watermarks`: per-resource cursors (updated_at, etag, last_modified).
- `ingestion_gaps`: detected pagination gaps or discontinuities.
- `qa_reports`: summary QA results for each run.
...
For per-repo storage, a common default path layout is:
