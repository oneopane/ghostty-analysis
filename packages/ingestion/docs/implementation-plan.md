...
- [x] Commits backfill (commit list + metadata, SHAs only).
- [x] Refs/releases backfill (branches, tags, releases as markers).
- [x] Incremental updates + watermarks.
- [x] Conditional requests (ETag / If-Modified-Since).
- [x] QA reporting / gap detection for missing pages or discontinuities.
- [x] Complete event payload coverage (map all GitHub issue/PR timeline events to canonical events).

### Implementation Notes
- Backfill now covers repo metadata, issues, PRs, reviews, review comments, issue comments, issue events, commits, branches, tags, and releases.
- Incremental updates use watermarks and conditional requests where available, and rebuild intervals for updated issues/PRs only.
- QA reports are produced after each run with gap counts by resource.
