...
def _():
    ...
    return BoundaryMembershipConfig, build_boundary_membership_dataset, datetime, fit_boundary_membership_model, mo, timezone
...
@app.cell
def _(BoundaryMembershipConfig, lookback_days, n_components):
    cfg = BoundaryMembershipConfig(
...
@app.cell
def _(build_boundary_membership_dataset, cfg, cutoff_dt, data_dir, mo, repo):
    if cutoff_dt is None:
        mo.md("Invalid cutoff datetime")
        dataset = None
    else:
        try:
            dataset = build_boundary_membership_dataset(
                repo=repo.value,
                cutoff=cutoff_dt,
...
@app.cell
def _(cfg, cutoff_dt, data_dir, fit_boundary_membership_model, mo, repo):
    if cutoff_dt is None:
        model = None
    else:
        try:
...
def _(model):
    ...
    # Show top boundaries for each role.
    top = {}
    for role, boundary_map in model.role_boundary_mix.items():
        top[role] = sorted(boundary_map.items(), key=lambda kv: (-kv[1], kv[0]))[:8]
    top
